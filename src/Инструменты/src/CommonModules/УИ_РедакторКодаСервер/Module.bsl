...
Функция ПараметрыФормированияРедакторов() 
	...
	Если ТипЗнч(ПараметрыСеансаВХранилище) = Тип("Структура") Тогда
		Если ПараметрыСеансаВХранилище.Свойство("ПолеHTMLПостроеноНаWebkit")
			И ЗначениеЗаполнено(ПараметрыСеансаВХранилище.ПолеHTMLПостроеноНаWebkit) Тогда
			ПарамерыФормирования.ПолеHTMLПостроеноНаWebkit = ПараметрыСеансаВХранилище.ПолеHTMLПостроеноНаWebkit;
		КонецЕсли;
		
		Если ПараметрыСеансаВХранилище.Свойство("ЭтоWindowsКлиент")
			И ЗначениеЗаполнено(ПараметрыСеансаВХранилище.ЭтоWindowsКлиент) Тогда
			ПарамерыФормирования.ЭтоWindowsКлиент = ПараметрыСеансаВХранилище.ЭтоWindowsКлиент;
		КонецЕсли;
		
		Если ПараметрыСеансаВХранилище.Свойство("ЭтоВебКлиент")
			И ЗначениеЗаполнено(ПараметрыСеансаВХранилище.ЭтоВебКлиент) Тогда
